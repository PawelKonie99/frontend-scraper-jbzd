...
import { AddMemeContainer, InputFile, Image, AddMemeForm, Input, MissingValueError, Success, BackToMain } from "./AddMemeElements";
    ...
    return (
        <AddMemeContainer>
            ...
            <AddMemeForm encType="multipart/form-data" onSubmit={handleAddMeme}>
                <Input placeholder="Dodaj tytuł" type="text" value={title || ""} onChange={({ target }) => handleAddTitle(target)} />
                <Image src={image} alt="" />
                <InputFile name="newMeme" type="file" id="input" accept="image/*" onChange={(event) => handleAddImage(event)}></InputFile>
                <Button type="submit" text={"Dodaj mema"}></Button>
            </AddMemeForm>
            {missingImageErr ? <MissingValueError>Brakuje obrazka !</MissingValueError> : ""}
            {missingTitleErr ? <MissingValueError>Brakuje tytułu !</MissingValueError> : ""}
